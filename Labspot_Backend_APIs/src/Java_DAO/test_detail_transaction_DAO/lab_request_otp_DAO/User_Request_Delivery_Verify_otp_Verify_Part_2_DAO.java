...
public class User_Request_Delivery_Verify_otp_Verify_Part_2_DAO
{
	public String user_Verify_otp_To_Verify_Delivery_Person_Part_2(User_Request_Delivery_Verify_otp_Verify_Part_2_Bean bean)
	{
		try
		{	
			...
			if(resultSet.next())
			{
				String otp = resultSet.getString("applicant_confirm_delivery_boy_otp_2");
				...
				if(otp.equals(bean.getOtp_from_delivery()))
				{
					String query1 = "UPDATE test_transaction_details SET "
							...
							+ "date_time_of_delivery_boy_arrive_2 = ? WHERE "
					...
					if(temp == 1)
					{
						return "Confirm Delivery Boy Part 2 OTP Verified";
					}
					else
					{
						return "Something went wrong in User Request Delivery Verify otp Verify Part 2";
					}
				}
				else
				{
					return "Confirm Delivery Boy Part 2 OTP is not Verified, Wrong OTP";
				}
			}
			else
			{
