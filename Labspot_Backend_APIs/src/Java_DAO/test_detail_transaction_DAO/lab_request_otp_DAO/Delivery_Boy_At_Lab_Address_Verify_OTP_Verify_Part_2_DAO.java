...
public class Delivery_Boy_At_Lab_Address_Verify_OTP_Verify_Part_2_DAO
{
	public String delivery_Boy_At_Lab_Address_Verify_OTP_Verify_Part_2(Delivery_Boy_At_Lab_Address_Verify_OTP_Verify_Part_2_Bean bean)
	{
		try
		{	
			...
			if(resultSet.next())
			{
				String otp = resultSet.getString("lab_confirm_delivery_boy_otp_2");
				...
				if(otp.equals(bean.getOtp_from_delivery()))
				{
					String query1 = "UPDATE test_transaction_details SET "
							...
							+ "date_time_of_delivery_boy_collect_report_2 = ? WHERE "
					...
					if(temp == 1)
					{
						return "Delivery Boy At Lab Part 2 OTP Verified";
					}
					else
					{
						return "Something went wrong in Delivery Boy At Lab Address Verify OTP Verify Part 2";
					}
				}
				else
				{
					return "Delivery Boy At Lab Part 2 OTP is not Verified, Wrong OTP";
				}
			}
			else
			{
				return "OTP to Verify Delivery Person Part 2 has not been generated yet, please generate OTP first then verify the OTP";
			}
		}
		catch(Exception e)
		{
