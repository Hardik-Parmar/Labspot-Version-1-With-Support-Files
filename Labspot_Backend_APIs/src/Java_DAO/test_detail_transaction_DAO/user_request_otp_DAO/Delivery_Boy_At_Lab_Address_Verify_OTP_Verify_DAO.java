...
public class Delivery_Boy_At_Lab_Address_Verify_OTP_Verify_DAO
{
	public String delivery_Boy_At_Lab_Address_Verify_OTP_Verify(Delivery_Boy_At_Lab_Address_Verify_OTP_Verify_Bean bean)
	{
		try
		{	
			...
			if(resultSet.next())
			{
				String otp = resultSet.getString("lab_confirm_delivery_boy_otp_1");
				...
				if(otp.equals(bean.getOtp_from_delivery()))
				{
					String query1 = "UPDATE test_transaction_details SET "
							...
							+ "date_time_of_delivery_boy_submit_sample_1 = ? WHERE "
					...
					if(temp == 1)
					{
						return "Delivery Boy At Lab OTP Verified";
					}
					else
					{
						return "Something went wrong in Delivery Boy At Lab Address Verify OTP Verify";
					}
				}
				else
				{
					return "Delivery Boy At Lab OTP is not Verified, Wrong OTP";
				}
			}
			else
			{
