...
public class Delivery_User_Request_Reject_DAO 
{
	public String delivery_User_Reject_Delivery_Request(Delivery_User_Request_Reject_Bean bean)
	{
		try
		{	
			Connection connection = ConnectionProvider.getConnection();
			
			String query = "UPDATE test_transaction_details SET delivery_boy_accept_1 =  ?"
					+ ", delivery_boy_name_1 = ?, delivery_boy_phone_1 = ?, "
					+ "delivery_boy_email_1 = ?, delivery_boy_address_1 = ? WHERE "
			...
			if(temp == 1)
			{
				return "Delivery Request from User is Rejected Successfully";
			}
			else
			{
				return "Delivery Request from User is not Rejected";
			}
		}
		catch(Exception e)
		{
		...
		return "Something went wrong in Delivery User Request Reject";
	}
}